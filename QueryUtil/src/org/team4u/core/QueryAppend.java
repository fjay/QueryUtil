...
public class QueryAppend {
	...
	private String appendCondictions() {
		...
		for (QueryCondiction condictionsBean : paramMap.values()) {
			...
			// 处理 "is (not) null" 条件
			if (StringUtil.containsIgnoreCase(condictionsBean.getOperation(), SysConstants.OPERATON_IS)
					&& (condictionsBean.getValue() == null || StringUtil.equalsIgnoreCase(
							condictionsBean.getOperation(), SysConstants.OPERATON_NULL))) {
	...
	private void wrap(StringBuffer whereBuilder, QueryCondiction condiction, Object value) {
		whereBuilder.append(condiction.getProperty()).append(" ").append(condiction.getOperation())
				.append(" ").append(value == null ? condiction.getValue() : value).append(" ");
	}
